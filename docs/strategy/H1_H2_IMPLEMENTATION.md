...
- Morgonkvittot visar högst tre öppna beslut, äldst först och högst belopp vid samma tidpunkt. Utgångna beslut finns i det beständiga kvittot. Övervakade utskicksförsök redovisas var för sig med känt eller okänt utfall; förtjänad autonomi summerar lyckade försök per typ.
...
- `pending_approvals.card_kind` härleds i en definer-trigger med låst search_path. Klassificeringen följer befintligt `ACTION_CONTRACT`. Beslut får sju dagar, autonomierbjudanden fjorton; information får ingen utgång. Befintliga väntande beslut normaliseras också.
...
2. Briefens checklist/microsite/debrief-exempel klassas efter faktisk befintlig handling. En typ som fäster en checklista eller skapar ett dokument förblir ett beslut. Endast `INFORMATIONAL` plus ren `agent_observation` är notices. Kontraktstestet kräver explicit klassificering av alla verkliga producenter.
