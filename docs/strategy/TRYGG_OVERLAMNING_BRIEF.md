...
| **Three decisions a day.** The morning push (existing `push-morgon` / `tyst-tid`) shows at most three open decisions; the rest wait. Order: cards with a deadline first (soonest `expires_at`, then highest money, then age), never-expiring internal gates only in a remaining slot, by age. Without that rule the gates, which only get older, would hold all three slots every day and a week-old customer send would never appear. A decision is answerable from the push with "Ja / Nej" where the card type supports it (start with `send_sms` and `invoice_reminder`). | Median pending age 6.8 days says the pile is the problem, not the decisions. The craftsman's channel is the phone. |
| **Expiry windows by type, not one default.** A deadline belongs only on a card whose action reaches the customer or moves money: 7 days. Offers of autonomy: 14 days (unchanged). Notices: none. **Internal work gates: none** (owner decision 2026-09-15) — `four_eyes_quote`, `four_eyes_project_close`, `review_auto_invoice`, `time_attestation`, `tidrapport_forslag`, `checklist_forslag`, `egenkontroll_foto`, `egenkontroll_avvikelse`, `job_report`, `lead_review`, `installation_register`, `manual_project_create`, `project_debrief`, `karin_deadline`. | 48-hour windows on pipeline reviews expire before a weekend is over. Silently dropping a four-eyes check or a statutory acknowledgement is worse than an old card in the list; an internal gate nobody answered stays open. |
