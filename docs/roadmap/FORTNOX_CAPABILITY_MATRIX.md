...
|---|---|---|---|---|---|---|
| Kundfakturor | Skapa, skicka, kreditera, PDF, från offert/projekt/tidrapport | **Byggd + testad**, men ROT-basen är fel på fakturasidan: `lib/invoice-calculations.ts` och fem vägar till räknar ROT på hela raden eller på restid. Se [artikel-/ROT-briefen](../strategy/ARTIKLAR_MALLAR_ROT_BRIEF.md) | Obligatorisk delning arbete/material/resa per rad, ROT bara på arbetsdelen, `rot_work_cost` på varje väg, Fortnox-raddelning (v252, brief 2026-09-16). Konteringen saknas (rad B1) | Codex | 27 SQL-kontroller + 8 specar i briefens §6 | September |
