...
for (const width of [375,1280]) test(`jobbval till standardrader och återanvändning ${width}px`, async ({ page }) => {
  ...
  try {
    ...
    await page.getByRole('button',{name:'Installera laddbox',exact:true}).click()
    await page.getByRole('button',{name:'Byta elcentral',exact:true}).click()
    await page.getByRole('button',{name:'Felsökning el',exact:true}).click()
    ...
    await expect(page.getByRole('button',{name:'Service på landet',exact:true})).toBeVisible()
    await page.getByRole('button',{name:'Fortsätt',exact:true}).click()
    await page.getByRole('button',{name:'Förbered standardrader för Installera laddbox',exact:true}).click()
    ...
    await page.getByRole('button',{name:'Lägg till rad',exact:true}).click()
    const preview=page.getByRole('region',{name:'Så här börjar nästa offert för Installera laddbox'})
