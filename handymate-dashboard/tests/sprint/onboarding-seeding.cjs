...
;(async()=>{
 let count=0
 for(const table of ['v3_automation_rules','lead_scoring_rules','pipeline_stage','quote_standard_texts','products','quote_templates']){
 ...
 for(const table of ['v3_automation_rules','pipeline_stage','quote_standard_texts','products','quote_templates','rpc']){
 ...
 // main serves standard checklists from code in GET /api/checklists/templates.
 // Seeding copies would duplicate them and must not block onboarding.
 assert(!h.state.reads.includes('checklist_template'),'checklist defaults must not depend on a database read')
 assert(!h.state.writes.some(w=>w.table==='checklist_template'),'checklist defaults must not be copied into company rows')
 count++;console.log('PASS checklist standards are not seeded or read during onboarding')
