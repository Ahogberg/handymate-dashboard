...
test.describe('first-job continuity in the real component', () => {
  test.describe.configure({ mode: 'serial' })
  let dom: any, root: any, host: HTMLElement, originalFetch: typeof fetch
  const originals = new Map<string, PropertyDescriptor | undefined>()
  const { act } = require('react-dom/test-utils'),
    { createRoot } = require('react-dom/client')
  const { WorkSampleResume } = c5Modules({})(
    'components/onboarding/WorkSampleResume.tsx',
  )
  const workId = '00000000-0000-0000-0000-000000000001'
  const sample = {
    version: 1,
    workId,
    source: 'Byt sex innerdörrar.',
    title: 'Byte av dörrar',
    description: 'Montering.',
    createdAt: '2026-09-08T10:00:00Z',
    items: [
      { description: 'Montera dörrar', quantity: 6, unit: 'st', type: 'labor' },
    ],
  }
  test.beforeEach(() => {
    const { JSDOM } = require('jsdom')
    dom = new JSDOM('<div id="root"></div>', { url: 'https://value.test/' })
    for (const key of [
      'window',
      'self',
      'document',
  ...
  test('unsaved application survives remount; saved work opens same quote without duplicating it', async () => {
    let linked = false
    const applied: any[] = []
    global.fetch = (async (url: any) =>
      Response.json(
        String(url).endsWith('/first-work')
          ? {
              receipt: {
                id: workId,
                business_id: 'a',
                quote_id: linked ? 'saved-quote' : null,
              },
            }
          : { business_id: 'a', onboarding_data: { workSample: sample } },
      )) as typeof fetch
    const render = (key: string) =>
      act(async () =>
        root.render(
          React.createElement(WorkSampleResume, {
            key,
            businessId: 'a',
            hasContent: false,
            onApply: (s: any) => applied.push(s),
            onSource: () => {},
          }),
        ),
      )
    ...
    expect(applied[0].workId).toBe(workId)
    await render('reload')
    expect(host.textContent).toContain('Använd mitt förberedda underlag')
    linked = true
    await render('saved')
    expect(host.querySelector('a')?.getAttribute('href')).toBe(
      '/dashboard/quotes/saved-quote',
    )
    expect(host.querySelector('button')).toBeNull()
    expect(applied).toHaveLength(1)
  })
  test('failed continuity read blocks reinserting the source until retry verifies no saved quote', async () => {
    let failing = true
    global.fetch = (async (url: any) =>
      String(url).endsWith('/first-work')
        ? failing
          ...
          : Response.json({
              receipt: { id: workId, business_id: 'a', quote_id: null },
            })
        : Response.json({
            business_id: 'a',
            onboarding_data: { workSample: sample },
          })) as typeof fetch
    await act(async () =>
      root.render(
        React.createElement(WorkSampleResume, {
    ...
    expect(host.textContent).not.toContain('Använd mitt förberedda underlag')
    failing = false
    await act(async () => host.querySelector('button')!.click())
    expect(host.textContent).toContain('Använd mitt förberedda underlag')
  })
