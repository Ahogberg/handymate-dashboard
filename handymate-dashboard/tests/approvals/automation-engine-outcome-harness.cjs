...
vm.runInNewContext(ts.transpileModule(fs.readFileSync(path.join(__dirname,'../../lib/automation-engine.ts'),'utf8'),{compilerOptions:{module:ts.ModuleKind.CommonJS}}).outputText,
 {module:mod,exports:mod.exports,require:name=>['@/lib/autonomy/earned-autonomy','@/lib/approvals/automation-message'].includes(name)?req(name):{},Date,Math,console,process:{env:{}},fetch:()=>{throw Error('No provider calls allowed')}})
