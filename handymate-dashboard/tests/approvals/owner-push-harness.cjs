...
const runtimeEnv={NEXT_PUBLIC_VAPID_PUBLIC_KEY:'mock-public',VAPID_PRIVATE_KEY:'mock-private'}
let tables,expoMode='ok',webMode='ok',calls=[],lostReceipt=false,dropReceipt=false,failClaim=false
function load(file){file=path.resolve(file);if(cache[file])return cache[file];const mod={exports:{}};vm.runInNewContext(ts.transpileModule(fs.readFileSync(file,'utf8'),{compilerOptions:{module:ts.ModuleKind.CommonJS,target:ts.ScriptTarget.ES2022}}).outputText,{module:mod,exports:mod.exports,Date,console,Response,process:{env:runtimeEnv},fetch:async(url,init)=>{
 assert.equal(url,'https://exp.host/--/api/v2/push/send');const payload=JSON.parse(init.body);assert.equal(payload.length,1);calls.push({channel:'expo',payload});if(expoMode==='network')throw Error('lost');if(expoMode==='http')return Response.json({}, {status:500});return Response.json({data:[expoMode==='ok'?{status:'ok',id:'ticket-1'}:expoMode==='bad' ? {status:'ok'} : {status:'error'}]})
},require:name=>{
...
const db={from(table){let op='read',values,filters=[],single=false;const chain=new Proxy({}, {get(_,key){
 if(key==='then')return resolve=>{
  ...
  if(op==='update'){if(failClaim&&values.status==='sending')return resolve({data:[],error:null});if(dropReceipt&&values.status==='accepted')return resolve({data:null,error:{message:'receipt write failed'}});selected.forEach(r=>Object.assign(r,structuredClone(values)));if(lostReceipt&&values.status==='accepted'){lostReceipt=false;return resolve({data:null,error:{message:'lost journal response'}})}}
...
function reset(){tables={business_users:[{id:'u1',business_id:'b1',user_id:'auth1',role:'owner',is_active:true,name:'Ägaren'}],push_tokens:[{id:'e1',business_id:'b1',user_id:'auth1',token:'ExponentPushToken[test]'}],push_subscriptions:[{id:'w1',business_id:'b1',user_id:'auth1',endpoint:'https://push.test/secret',p256dh:'secret-key',auth:'secret-auth'}],v3_automation_logs:[]};expoMode='ok';webMode='ok';calls=[];lostReceipt=false;dropReceipt=false;failClaim=false;runtimeEnv.VAPID_PRIVATE_KEY='mock-private'}
...
;(async()=>{
 ...
 reset();const changed=await prepare(db,'b1','a1',payload);tables.push_tokens[0].token='replacement';result=await execute(db,'b1','a1',changed.executionPayload);assert.equal(result.partial,true);assert.equal(calls.filter(c=>c.channel==='expo').length,0)
 reset();tables.push_tokens=[];tables.push_subscriptions=[];await assert.rejects(()=>prepare(db,'b1','a1',payload),/saknar registrerade/);assert.equal(calls.length,0)
 reset();const missingConfig=await prepare(db,'b1','a1',payload);delete runtimeEnv.VAPID_PRIVATE_KEY;result=await execute(db,'b1','a1',missingConfig.executionPayload);assert.equal(result.partial,true);assert.equal(calls.length,1);runtimeEnv.VAPID_PRIVATE_KEY='mock-private';assert.equal((await execute(db,'b1','a1',(await prepare(db,'b1','a1',payload)).executionPayload)).ok,true);assert.equal(calls.filter(c=>c.channel==='expo').length,1)
 reset();const noClaim=await prepare(db,'b1','a1',payload);failClaim=true;assert.equal((await execute(db,'b1','a1',noClaim.executionPayload)).ok,false);assert.equal(calls.length,0)
 reset();const dropped=await prepare(db,'b1','a1',payload);dropReceipt=true;assert.equal((await execute(db,'b1','a1',dropped.executionPayload)).partial,true);dropReceipt=false;assert.equal((await execute(db,'b1','a1',(await prepare(db,'b1','a1',payload)).executionPayload)).partial,true);assert.equal(calls.length,2)
 reset();expoMode='http';const http=await prepare(db,'b1','a1',payload);await execute(db,'b1','a1',http.executionPayload);await execute(db,'b1','a1',(await prepare(db,'b1','a1',payload)).executionPayload);assert.equal(calls.length,2)
