...
;(async()=>{
 ...
 try {
  const page=await browser.newPage();let posts=[],attachmentStatus=200
  ...
  await page.route('**/*',async route=>{
   ...
   if(url.endsWith('/api/approvals/a')){const body=route.request().postDataJSON();posts.push(body);return route.fulfill({status:body.action==='preview'?428:200,json:body.action==='preview'?{review,review_token:'test-proof'}:{receipt:{state:'queued',text:'Köat'}}})}
