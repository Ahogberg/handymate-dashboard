...
test('actual HTTP route denies unauthenticated access and uses server identity, date, no-store',async()=>{
 ...
 const make=(business:any,user:any)=>{const m={exports:{} as any};new Function('require','exports','module',code)((name:string)=>{if(name==='next/server')return {NextRequest,NextResponse};if(name==='@/lib/auth')return {getAuthenticatedBusiness:async()=>business};if(name==='@/lib/permissions')return {getCurrentUser:async()=>user};if(name==='@/lib/supabase')return {getServerSupabase:()=>database().db};if(name==='@/lib/matte/report-session')return require('../lib/matte/report-session');if(name==='@/lib/relief/my-day')return {loadMyDay};if(name==='@/lib/relief/day-summary')return {loadDaySummary:()=>{throw Error('wrong branch')}};if(name==='@/lib/matte/work-report')return require('../lib/matte/work-report');throw Error(name)},m.exports,m);return m.exports}
