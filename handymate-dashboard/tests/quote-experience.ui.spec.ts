...
test('internal navigation flushes a recovery copy before the debounce elapses', async ({page}) => {
  await mount(page, 'recovery')
  await page.getByLabel('Arbete').fill('Senaste ändringen före navigering')
  await page.evaluate(() => (window as any).remount())
  await page.getByRole('button', {name:'Återställ', exact:true}).click()
  await expect(page.getByLabel('Arbete')).toHaveValue('Senaste ändringen före navigering')
})

test('an older autosave finishes before the explicit latest quote save', async ({page}) => {
  await mount(page, 'recovery')
  const code = ts.transpileModule(fs.readFileSync('app/dashboard/quotes/_shared/useQuoteBuilderSave.ts','utf8'), {compilerOptions:{module:ts.ModuleKind.CommonJS,target:ts.ScriptTarget.ES2020}}).outputText
  await page.addScriptTag({content:`
    window.writes=[]; window.resolveWrite=[];
    window.fetch=(_,options)=>new Promise(resolve=>{window.writes.push(JSON.parse(options.body));window.resolveWrite.push(()=>resolve({ok:true,json:async()=>({})}))});
    const saveModule={exports:{}};
    new Function('require','module','exports',${JSON.stringify(code)})(n=>n==='react'?React:{buildQuotePayload:c=>({title:c.title,items:c.items})},saveModule,saveModule.exports);
    function SerialHost(){
      const [title,setTitle]=React.useState('Äldre titel');
      const h=saveModule.exports.useQuoteBuilderSave({mode:'edit',quoteId:'q',items:[],setItems:()=>{},products:[],setLocalPrice:()=>{},setSendConfirmPending:()=>{},toast:{warning:()=>{},error:()=>{},success:()=>{}},router:{push:()=>{}},getContext:()=>({title,paymentPlan:[],description:'Arbete'}),onSaved:()=>window.saved=true});
      return React.createElement('div',null,React.createElement('input',{'aria-label':'Titel',value:title,onChange:e=>setTitle(e.target.value)}),React.createElement('button',{onClick:()=>h.performAutoSave()},'Autospara'),React.createElement('button',{onClick:()=>h.save(false)},'Spara senaste'));
    }
    const el=document.createElement('div');document.body.appendChild(el);ReactDOM.createRoot(el).render(React.createElement(SerialHost));
  `})
  await page.getByRole('button',{name:'Autospara',exact:true}).click()
  await expect.poll(()=>page.evaluate(()=>(window as any).writes.length)).toBe(1)
  await page.getByLabel('Titel',{exact:true}).fill('Senaste titel')
  await page.getByRole('button',{name:'Spara senaste',exact:true}).click()
  await page.getByRole('button',{name:'Autospara',exact:true}).click()
  expect(await page.evaluate(()=>(window as any).writes.length)).toBe(1)
  await page.evaluate(()=>(window as any).resolveWrite[0]())
  await expect.poll(()=>page.evaluate(()=>(window as any).writes.length)).toBe(2)
  expect(await page.evaluate(()=>(window as any).writes.map((v:any)=>v.title))).toEqual(['Äldre titel','Senaste titel'])
  await page.evaluate(()=>(window as any).resolveWrite[1]())
  await expect.poll(()=>page.evaluate(()=>(window as any).saved)).toBe(true)
})
