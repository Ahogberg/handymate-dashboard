...
| Färgdrift på kläder mellan tagningar (hoodien 11° för blå i närbilden) | Mät hue på ett klädparti i referenstagningen och i den nya (PIL i sandboxen), sök fram `selectivecolor` på cyan-området med huden som vakt, lägg på segmentet i ihopsättningen. 0 kr. |
| Rekvisita "dyker upp" i stället för att röra sig | Skriv fysiken som kameran ska se: objektet, hjulen, benen, riktning, tyngd, och att det lämnar bilden igen. Element för båda karaktärerna. |
| Seedance med audio_references levererar ett eget ljudspår — en förvrängd version av referensen ("På 10 år?") | Använd ALDRIG det inbakade ljudet, inte ens i utkast. Lägg originalfilen (TTS/inspelning) rakt över tagningen vid ihopsättning; Sync Lipsync i final. |
| Textremsor i drawtext ser "ffmpeg" ut | Grafik (namnkort med porträtt, tidsplatta, pratbubbla) renderas i PIL i 2x och skalas ned; animeras med overlay-uttryck (slide) och fade=alpha. Kortens in-tider läses ur VO:n med silencedetect. 0 kr. |
| Ljud: musik och effekter ad hoc per film | **Handymate-ljudsystemet** (`reference-pack/assets/audio/`, regelblad i txt): rumsljud under tystnad, klocka-stopp som skämtverktyg, bädden in först när skämtet landat (stems bygger per person), godkänn-snäpp på beslutet, outro på slutraden, signaturen alltid ensam på plattan. Full-mix 120 = standardspår för socialt innehåll; stems bara när du vet varför. Leverans −16 LUFS. |

## Regel (F14, 2026-08-29): repliker utan ljudreferens
Seedance 2.0 med `audio_references` (Benji) ger Matte ett leende i tagningar där han fyller bilden (B8b: 3 av 3 försök, även med "never smiles" i prompten). Generera Matte-repliker **utan** ljudreferens ("says one short dry sentence, flat expression") och lägg läpparna med Sync efteråt — Sync är ändå slutsteget. Ljudreferens bara när ansiktet är litet i bild eller när tonen ska synas.
