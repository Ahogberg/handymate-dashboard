...
|---|---|---|---|
| A1 | Offert avvisas | 🔴 | **Riktig bugg hittad, ej fixad än — kräver Andreas "kör".** Fyra-ögon-grinden för offerter kan ALDRIG lyckas: `app/api/quotes/send/route.ts` skriver `status='pending_approval'` när en icke-ägare skickar över tröskeln, men `quotes_status_check` (DB) tillåter bara draft/sent/opened/accepted/declined/expired — CHECK-brott (23514), fångat och returnerat som ärligt 500 ("Godkännande skapad men offertens status kunde inte uppdateras"). Godkännandekortet skapas ändå, men offerten låses aldrig mot dubbelskick. `tests/quote-lifecycle.spec.ts` bekräftar att `pending_approval` REDAN är appens uttalade kontrakt (klassificerat som avsiktligt arbetsläge, med en kommentar om en TIDIGARE liknande incident) — det är databasen som aldrig synkades, inte koden som är fel. Migration skriven: `sql/v167_quote_pending_approval_status.sql` (lägger till `'pending_approval'` i constraint). Se Avvikelseloggen #16. |
