...
### 2026-09-08 — time_attestation, tidrapport_forslag, dispatch_suggestion: stored-result verification
- Attestation verifies the existing time row's date, person, project, minutes, billable flag and approval status. It reads back inserted time and updated check-in even after response loss; a completed retry preserves the original check-in approval timestamp. Changed check-in project and noninteger minutes are refused.
- Time proposals validate real calendar dates and integer minutes. Existing stable artifacts must match date/person/project/minutes/billing/approval before success. Dispatch reads the stored assignee and booking member ID before success; missing employee names fail. Work-order date and end time are now displayed as well.
- Verified actual public table columns through read-only information_schema query. Work orders have name/phone assignment, no assigned_user_id; concurrency/phone reassignment semantics still need dedicated review.
- Seven isolated internal-write tests pass, including response loss, no duplicate time row, preserved timestamp and modified billing/date/status refusal. Real route harness now runs time proposals through actual artifact writer and person resolver: invalid date, explicit CTA, exact saved approved/billable row, persisted receipt and duplicate decision. Existing route regressions pass. Typecheck passes.
- These are isolated tests, NOT live or iPhone certification. Next: synthetic live fixtures for these three and both dispatch target variants; review check-in status/date provenance, compare-and-set assignment, work-order assigned_phone, lost time-proposal response recovery and conflict repair. No new production/test-account writes in this checkpoint. Build 12 unchanged.

Follow-up: all three synthetic web base flows now completed on dbf5542f, including cancel/no writes, actual stored time/assignment and matching history after reload. Exact IDs, evidence and remaining variants in approval-live-2026-09-08.md. This supersedes the checkpoint's pending live-base step, not its outstanding variant/native gaps.

### 2026-09-08 — dispatch immutable retry plan and conditional assignment
- Review now signs and persists dispatchPlan (target, member, original assignment, intended assignment). Retry uses that original plan, never adopts a newer assignment as its starting state.
- Executor verifies member and target; already-correct assignment returns success without writes. A different assignment is preserved. Conditional UPDATE includes original assigned_to and, for booking, assigned_user_id; readback reconciles a lost response. No schema changes.
- Eight isolated internal-write tests pass. Real booking-dispatch HTTP route tested signed stale preview denial, failure, stored original evidence, reviewed retry with lost response, actual member ID/name, persisted receipt, no duplicate success write and no overwrite of newer assignment. Full existing route harness passes; TypeScript check passes.
- This checkpoint has not been live clicked. Previous three live base proofs remain on dbf5542f. Next live booking fixture should use a cancelled/past synthetic booking to avoid reminders, verify preview/cancel/assignment/history on current preview. Work-order phone consistency, native variants and legacy failed cards without dispatchPlan remain unresolved. Blocking an ambiguous legacy retry is not a completed repair flow.

### 2026-09-08 continuation: booking live assignment + three mobile review variants
Booking dispatch path live verified on old cancelled synthetic booking: cancel/no write, correct name + member ID, stored dispatch plan and identical history after reload. Added explicit status display after noticing status absent in preview; actual route harness and typecheck pass. Mobile review tests expanded from 14 to 17 using real component/adapter with mocked servers. Activity remount, list and queue harnesses pass. Exact limitations and fixture in live audit. No new TestFlight build; active-booking lifecycle and remaining role/error/native cases are not certified.
