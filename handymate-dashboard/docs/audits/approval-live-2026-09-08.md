...
### Active booking assignment and stale-review recovery — live 2026-09-08
Three confirmed/scheduled synthetic booking variants now live-proved in explicitly authorized TEST Rollprov A: new assignment, replacement, and concurrent assignment change while review is open. Required isolated customer explicitly approved, no phone/email and sending opted out. Seed/conflict/recheck SQL and full evidence: docs/audits/approval-active-dispatch-2026-09-08.md.
Cancel preserved original assignments; final decisions stored correct employee name/member ID while preserving status and times. All three persisted saved receipts match Hanterade after full reload. Two pre-existing SMS cards untouched.
Live found broken recovery: a rejected stale pending decision used execution retry. Fixed error banner to reopen original approve/edit/reject review, preserving edits; genuine execution failures retain retry. Accurate generic review error text. On Vercel-success 9e7a1953, stale assignment was preserved, Granska på nytt showed the new assignment, and explicit final decision succeeded.
Actual page/banner harness, route harness (including six active cases), history harness, full tsc (8 GB after initial 4 GB OOM), and full build (327 pages/tracing) pass. Published application file hashes match tested local code.
Open: raw ISO dates in review, phone/legacy/race variants, full lifecycle/roles/native and other 77-type gaps. Build 12 unchanged; no main merge or production release.

### Work-order phone and legacy receipt recovery — 2026-09-08
Name and phone now reviewed/written/read back together; missing member phone clears the previous work-order phone. Phone-only changes are protected by signed snapshot and conditional update. Legacy missing-plan/phone-plan recovery only acknowledges an already matching assignment with before=after, no assignment writes; conflicts remain blocked.
Actual route tests cover phone changes, no phone, failed/lost-response retry, stale source phone, newer-phone conflict and legacy no-write recovery. Full tsc/build pass. On Vercel-success 3081badb, two scoped live work-order fixtures passed preview/cancel/final decision/history after reload: old non-dialable marker cleared with person change; existing legacy assignment preserved while failed receipt restored to success. No SMS or profile/customer writes. Full evidence and limits: docs/audits/approval-work-phone-2026-09-08.md. Next: time-proposal response-loss/edited-artifact recovery. Native and other 77-type gaps remain; build12 unchanged.

### Time-proposal response loss and existing-row review — 2026-09-08
Preview now reads the stable existing time row and binds it to the decision; matching rows say Bekräfta registrerad tid, edited rows/read errors block. Actual saved fields include description and are shared with the executor. Executor reads back the stable row after returned or thrown insert errors; no second insert in the same attempt.
Actual route harness covers lost responses, real failed insertion/retry, existing-row reuse, changed fields and stale snapshots. 38 existing time/internal-write tests, full tsc and full build pass. On Vercel-success 46718f9a, existing synthetic 75-minute card recovered from deliberately reconstructed failed receipt: preview/cancel/final decision, same one row and unchanged entire-row hash, durable receipt in history after reload. No extra wages/time row, SMS or external operation. Full evidence/remaining limits: docs/audits/approval-time-recovery-2026-09-08.md. Remaining internal work includes check-in date/status provenance, checklist variants and native; broader 77-type gaps remain. Build12 unchanged.
