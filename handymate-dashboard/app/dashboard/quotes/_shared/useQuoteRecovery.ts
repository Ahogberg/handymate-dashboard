...
export function useQuoteRecovery<T>({userId,businessId,scope,enabled,value,hasContent,onRestore}: {userId:string;businessId:string;scope:string;enabled:boolean;value:T;hasContent:boolean;onRestore:(value:T)=>void}) {
  ...
  useEffect(()=>{
    ...
    function persist(){
      if(disabled.current || !current.current.hasContent)return
      try {sessionStorage.setItem(key!,JSON.stringify({version:1,savedAt:Date.now(),value:current.current.value}));setStatus('Återställningskopia i den här fliken · spara utkast för att behålla på servern')}
      catch {setStatus('Återställningskopian kunde inte sparas. Spara utkast innan du lämnar sidan.')}
    }
