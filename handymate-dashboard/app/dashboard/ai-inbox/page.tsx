...
export default function AIInboxPage() {
  ...
  async function fetchData() {
    ...
    suggestionsByRecording.forEach((suggs, recordingId) => {
      ...
      // Fallback till recording customer
  ...
  const handleApprove = async (suggestion: AISuggestion) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result.error || 'Kunde inte godkänna förslag')
      }

      showToast(result.message || 'Förslag godkänt och utfört!', 'success')
      setEditingId(null)
      fetchData()
      ...
      <div className="relative">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 sm:mb-8">
          <div className="flex items-center">
            ...
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">AI Inbox</h1>
              <p className="text-gray-500">Förslag från samtalsanalys</p>
            </div>
          </div>
        </div>
                  ...
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex items-start gap-4 min-w-0 flex-1">
                        {/* Play button om recording finns */}
                        {group.recording && (
                          <button
                            onClick={(e) => {
                              e.stopPropagation()
                                  ...
                                  <div className="min-w-0 flex-1">
                                    ...
                                    {isEditing && (
                                      <div className="mt-4 p-4 bg-white rounded-xl space-y-3">
                                        <p className="text-sm text-gray-500 mb-2">Redigera innan godkännande:</p>
                                        ...
                                        {suggestion.suggestion_type === 'quote' && (
                                          <>
                                            <input
                                              type="text"
                                              value={editForm.service || ''}
                                        ...
                                        <div className="flex gap-2">
                                          <button
                                            onClick={cancelEdit}
                                            className="px-3 py-1.5 text-gray-500 hover:text-gray-900 text-sm"
                                          >
                                            Avbryt
                                          </button>
                                          <button
                                ...
                                {suggestion.status === 'pending' && !isEditing && (
                                  <div className="flex items-center gap-2">
                                    ...
                                    <button
                                      onClick={() => handleApprove(suggestion)}
                                      disabled={actionLoading === suggestion.suggestion_id}
                                      className="flex items-center gap-1 px-3 py-1.5 bg-emerald-100 border border-emerald-200 rounded-lg text-emerald-600 text-sm hover:bg-emerald-500/30 disabled:opacity-50"
                                    >
                                      {actionLoading === suggestion.suggestion_id ? (
                                        <Loader2 className="w-3 h-3 animate-spin" />
                                      ) : (
