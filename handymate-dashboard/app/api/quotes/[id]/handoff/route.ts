...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const [config, rules, settings, related, direct, entity, logs, legacyLogs, customer] = await Promise.all([
  ...
  if ([config, rules, settings, related, direct, entity, logs, legacyLogs, customer].some(result => result.error) || !config.data) {
