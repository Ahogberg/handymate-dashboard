...
async function getBusinessContext(businessId: string) {
  ...
  const [
    ...
    // Branschförståelse steg 1 — bransch + specialiteter + företagets egna
    // jobbtyper. Fail-soft (returnerar null själv vid fel).
...
function buildContextSection(ctx: Awaited<ReturnType<typeof getBusinessContext>>): string {
  ...
  if (ctx.tradeContext) {
    lines.push('')
    lines.push(formatTradeContextBlock(ctx.tradeContext))
  }
