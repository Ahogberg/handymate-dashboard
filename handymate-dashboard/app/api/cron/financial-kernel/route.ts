...
export async function GET(request: Request) {
  ...
  const summary = { businesses: 0, consumed: 0, swept: 0, halted: 0, unknown: 0, skipped: 0, budgetExhausted: false,
  ...
  for (const business of businesses) {
    ...
    try {
      ...
      for (const row of owed) {
        if (shouldStop()) break
        const sweep = await sweepInvoiceIntents(businessId, row.invoice_id, { db, sb })
      ...
      await rapporteraTystFel(sb, businessId, 'financial-kernel:runner-failed', message, { cron: 'financial-kernel' })
    }
  }
