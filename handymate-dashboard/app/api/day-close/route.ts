...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (request.nextUrl.searchParams.get('view') === 'reports') {
      if (!reportContinuityEnabled(business.business_id)) return NextResponse.json({enabled:false,reports:[]}, {headers})
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!reportContinuityEnabled(business.business_id)) return NextResponse.json({error:'Återupptagning är inte aktiverad.'},{status:503,headers})
    const user = await getCurrentUser(request,business.business_id)
