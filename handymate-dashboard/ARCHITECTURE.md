...
| Event | Triggas när | Primär payload | Paket | Finns |
|-------|-------------|----------------|-------|-------|
| `journal_entry_posted` | En verifikation är atomärt bokförd med balanserade rader. Verifikationsnumret tilldelas i samma transaktion (blueprint §36.1). | `{ journal_entry_id, journal_type, voucher_series, voucher_number, effective_date, period_id, rule_id, rule_version, source_event_id\|null }` | C8 | ✅ |
...
- `channel_notices` (v246): one service-created informational row per business/channel/Swedish day, no approval and no expiry; same Swedish reason as the home banner. Internal platform credit shortages are Handymate's responsibility.
- `MORNING_REPORT_RELIABILITY_ENABLED` (default off, v246 first): the exact seeded system morning rule uses existing `generateMorningBrief(...,{strict:true})`, without LLM/tool execution. It retains core query failures instead of presenting empty data as healthy. Existing custom `run_agent` rules remain on their own path and gain classified errors.
...
## Trygg överlämning — H1/H2 (2026-09-15, review)

`HANDOFF_INBOX_ENABLED` visar information utan svarskrav och sparade morgonkvitton. v248 normaliserar kortklassificering/utgång i databasen och samlar varje verklig expiry en gång. Utgång är en explicit typmatris: kund-/pengahandlingar sju dagar, autonomierbjudanden fjorton, notices och fjorton interna grindar ingen. Backfill kortar aldrig en levande svarstid. Morgonkvittot prioriterar närmast deadline; interna grindar fyller restplatser efter ålder. Digestnotices behåller även "Skött utan dig"-historiken. `SUPERVISED_AUTONOMY_ENABLED` kräver H1 och preflight samt uttryckligt engångssamtycke från ägare/admin. Fyra befintliga nycklar, oförändrade beloppsgränser, beständiga avstängningar och audit före leverantörsanrop. Signerad off är POST-only. Native Ja/Nej återstår som en separat mobil integration; fullständig H1/H2-avvikelselista, flaggordning och rollback finns i `../docs/strategy/H1_H2_IMPLEMENTATION.md`.

### H3b: beständiga utskick

`OUTBOUND_INTENTS_ENABLED` kopplar SMS, e-post och push till samma beständiga statusmaskin: källa och intent skrivs före leverantörsanropet, en token stängs efter anropet och ett tappat svar blir `unknown` utan automatiskt omutskick. `outbound_intents` innehåller bara mottagare, mall och källidentitet. `outbound_messages` är den tenantbundna, oföränderliga källan för meddelanden som tidigare bara fanns i processminnet; dokument lagrar en privat Storage-referens i stället för en andra PDF-kopia. Båda tabellerna är service-only, omfattas av kontoradering och skrivs endast via token-/låsskyddade RPC:er. Tiominuterscronen ger outbound ett eget tidsfönster före ekonomiarbetet. Ett återhämtat utskick stämmer av den ursprungliga fakturapåminnelsen, dokumentjournalen och autonomikvittensen exakt en gång. Avstängning tar låsen i ordningen `autonomy:` → `outbound:` och annullerar väntande försök i samma transaktion. Flaggan är av tills v249 har körts och pilotens kanaler har verifierats.
