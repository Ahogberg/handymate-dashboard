 ...
 * QuoteTemplateData som Modern-dokumentet. Ersätter BÅDE
 * components/quotes/editable/ModernCanvas.tsx (live-canvasen) OCH
 * lib/quote-templates/modern.ts (mallsträngen) — live-canvas, preview-HTML
 * och PDF renderas nu alla från DENNA komponent.
 *
 * Isomorf med flit: INGEN 'use client', INGA hooks. mode="static" körs
 * genom renderToStaticMarkup i lib/quote-templates/render-react.tsx (ren
 * Node-kontext, ingen webbläsare) — komponenten får därför aldrig referera
 * webbläsar-API:er eller state direkt. Edit-lägets interaktivitet kommer
 * enbart via props (handlers) + EditableText/EditableNumber-fälten (som
 * ÄR 'use client', men det är helt normalt att en icke-direktiv-markerad
 * komponent renderar client-komponenter — det är bara det omvända som
 * kräver en gräns).
 *
...
export default function QuoteDocument({ data, mode, handlers, sheetMode, onRowTap }: QuoteDocumentProps) {
  ...
  // ModernCanvas hade. Static-läget respekterar visningsnivån (Del C),
  // exakt som lib/quote-templates/modern.ts.
  const showQty = mode === 'edit' ? true : data.showQuantities !== false
  const showPrice = mode === 'edit' ? true : data.showUnitPrices !== false
  ...
  // behöver inte noten — hantverkaren väljer inte tillval.
  const hasOptions = data.quote.items.some(i => i.itemType === 'option')
  const showOptionsNote = mode === 'static' && hasOptions && !data.isSigned

  // Signatur-CTA (punkt 5): resolvad generisk default (isSigned-baserad)
  // om data.signatureCta är utelämnad. buildQuoteTemplateData sätter idag
  // ALLTID explicit 'hidden' i produktion — se kommentar i data-builder.ts.
  const resolvedCta = data.signatureCta ?? (data.isSigned ? 'signed' : 'active')
      ...
      <div className="page">
        ...
        <section className="parties">
          <div>
            ...
            {data.referencePerson && <div className="party-meta">Vår referens: {data.referencePerson}</div>}
          </div>
          <div>
            <div className="party-label">Mottagare</div>
            <div className="party-name">
              {mode === 'edit' && handlers?.onCustomerNameChange
...
/** Exakt samma default-villkorstext som lib/quote-templates/modern.ts skrev
    ...
    använder samma sanning. */
function defaultTermsText(data: QuoteTemplateData): string {
