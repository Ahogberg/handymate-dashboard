...
interface Receipt { token: string; label: string; summary: string; duplicate: boolean }
export default function DayClose({ projectId, projectName }: { projectId: string; projectName: string }) {
  ...
  async function send(confirm = false) {
    ...
    try {
      ...
      if (confirm) {
        ...
        setReceipts(previous => previous.some(row => row.token === current!.token) ? previous : [...previous, { token: current!.token, label: REPORT_LABELS[current!.tool_name], summary: current!.summary, duplicate: status === 'already_saved' }])
