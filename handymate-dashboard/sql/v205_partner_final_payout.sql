-- Partnerprogram: avtalstrogen slututbetalning under ordinarie 500-kronorsgrans.
-- Forutsatter v193 och v194. KORS MANUELLT i Supabase SQL Editor.
-- En slututbetalning ar en uttrycklig adminhandling med obligatoriskt skal;
-- den far inte bli ett tyst generellt undantag fran minimibeloppet.

BEGIN;

ALTER TABLE public.partner_payout_batch
  ADD COLUMN IF NOT EXISTS is_final_payout BOOLEAN NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS final_payout_reason TEXT;

ALTER TABLE public.partner_payout_batch
  DROP CONSTRAINT IF EXISTS partner_payout_batch_final_reason_check;
ALTER TABLE public.partner_payout_batch
  ADD CONSTRAINT partner_payout_batch_final_reason_check
  CHECK (
    (NOT is_final_payout AND final_payout_reason IS NULL)
    OR (is_final_payout AND COALESCE(BTRIM(final_payout_reason), '') <> '')
  );

DROP FUNCTION IF EXISTS public.create_partner_self_billing_batch(UUID, TEXT, JSONB, TEXT);

CREATE FUNCTION public.create_partner_self_billing_batch(
  p_partner_id UUID,
  p_period TEXT,
  p_buyer JSONB,
  p_actor TEXT,
