...
function failOnReadError(errors: Array<{ area: string; error: DbError }>): void {
  const failed = errors.find(item => item.error)
  if (!failed) return
  console.error(`[onboarding/channel-health] Kunde inte läsa ${failed.area}:`, failed.error)
  throw new Error('Kunde inte verifiera kundinflödets status')
}

/** Samma befintliga bevisfrågor för HTTP och cron. Anroparen ansvarar för
 * auth/cron-behörighet och väljer businessId; varje fråga filtrerar tenant.
 * Läsfel kastas: okänt underlag får aldrig bli en gissad kanalstatus. */
export async function loadChannelHealth(supabase: SupabaseClient, businessId: string) {
  if (!businessId) throw new Error('Företag saknas')
  const [
    configResult,
    emailRouteResult,
  ...
  if (!configResult.data) {
    ...
    throw new Error('Kunde inte läsa företagets inställningar')
  }
