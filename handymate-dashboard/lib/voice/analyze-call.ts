...
/**
 * ═══ ANALYSMOTORN — den SUGGESTING hjärnan i röstvägen (etapp 1b) ═══
 *
 * Flyttad ut ur app/api/voice/analyze/route.ts (HTTP-endpointen är kvar som
 * en tunn adapter) så att app/api/voice/transcribe kan anropa analysen
 * direkt, in-process, istället för ett HTTP-självanrop mot sin egen app.
 *
 ...
 * ═══ ANSPRÅKET (v155) ═══
 *
 * Innan v155 fanns bara en SELECT-före-INSERT-spärr mot ai_suggestion —
 * mötesgrenen (source = 'site_visit') skriver ALDRIG dit, den skriver
 * pending_approvals, så den spärren skyddade aldrig möten. Två samtidiga
 * analysanrop av samma nya möte kunde båda passera. claimCallAnalysis nedan
 * är ett atomiskt UPDATE ... WHERE analysis_claimed_at IS NULL — bara en
 * körning vinner, oavsett vilken typ av inspelning det gäller. Anspråket
 * fungerar också som den nya dubbelkörningsspärren generellt: en redan
 * klar analys lämnar analysis_claimed_at satt, så en andra (icke-force)
 * begäran nekas direkt istället för att räkna ai_suggestion-rader.
 *
 * Migrationen kan sakna att ha körts — claimCallAnalysis faller då tillbaka
 * på den gamla ai_suggestion-baserade already_analyzed-spärren (bevarad
 * nedan för exakt det syftet).
 */
 ...
 *           (`prompt` nedan) skulle ha gjort — resten av funktionen
 *           (metering, JSON-parsning, kortbygget) läser bara `response` och
 *           `analysisResult` och märker ingen skillnad mellan de två vägarna.
 */
...
Om inget konkret diskuterades i den här delen: returnera {"fynd": []}. Svara ENDAST med JSON, ingen annan text.`
  ...
  try {
    ...
    console.warn('[analyze-call] map-reduce chunk-parsning misslyckades:', parseErr)
  }
 ...
 * engångsanropet — anroparen behöver inte veta vilken väg som kördes.
 */
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    if (usage) {
      try {
        ...
        console.warn('[analyze-call] map-reduce kostnadsmätning misslyckades:', costErr)
      }
    }
  }
...
/** Resultatet av analyseraSamtal — HTTP-adaptern (app/api/voice/analyze/route.ts)
    översätter varje status till rätt svarsform. */
export type AnalyseraSamtalResult =
  | {
      status: 'not_found'
      recording_id: string
      debug: { recording_id: string; fetchError: string | null; fetchCode: string | null }
    }
  | { status: 'no_transcript'; recording_id: string }
  | { status: 'globally_paused'; recording_id: string; message: string }
  | { status: 'already_claimed'; recording_id: string; message: string }
  | { status: 'already_analyzed'; recording_id: string; summary: string | null; suggestions: any[] }
  | { status: 'meeting_done'; recording_id: string; summary: string | null; cards_created: number }
  | {
      status: 'phone_done'
      recording_id: string
      ...
      auto_approve_results: any[]
    }
  | { status: 'error'; recording_id: string; error: string }

/**
 * Tar ett atomiskt anspråk på analysen av ett samtal/möte.
 *
 * force=true rensar först ett ev. befintligt anspråk (och analyzed_at) innan
 * det tar ett nytt — det är hur "Analysera igen" kringgår ett tidigare
 * slutfört (eller kvarhängande) anspråk.
 *
 * Saknas v155 (kolumnen finns inte) faller vi tillbaka på den gamla
 * ai_suggestion-baserade already_analyzed-spärren — se anroparen nedan.
 */
async function claimCallAnalysis(
  supabase: SupabaseClient,
  recordingId: string,
  force: boolean
): Promise<{ claimed: boolean; migrationMissing: boolean }> {
  if (force) {
    const { error: clearErr } = await supabase
      .from('call_recording')
      .update({ analysis_claimed_at: null, analyzed_at: null })
      .eq('recording_id', recordingId)
    if (clearErr) {
      if (isMissingColumnError(clearErr)) return { claimed: false, migrationMissing: true }
      throw clearErr
    }
  }
  ...
  if (error) {
    if (isMissingColumnError(error)) return { claimed: false, migrationMissing: true }
    throw error
  }

  return { claimed: (data?.length ?? 0) > 0, migrationMissing: false }
}

/**
 * Analyserar ett transkriberat samtal/möte med AI och skapar förslag.
 * Ren funktion — tar en service-role/behörig Supabase-klient, ingen
 * NextRequest/NextResponse. HTTP-adaptern (route.ts) sköter auth,
 * tenantmatchning och svarsform.
 */
export async function analyseraSamtal(params: {
  supabase: SupabaseClient
  recordingId: string
  force?: boolean
}): Promise<AnalyseraSamtalResult> {
  const { supabase, recordingId: recording_id, force = false } = params
  ...
  try {
    ...
    // address does not exist" (42703) på VARJE anrop.
    ...
    if (fetchError || !recording) {
      console.error('[analyze-call] Recording not found:', { recording_id, fetchError: fetchError?.message, fetchCode: fetchError?.code })
      return {
        status: 'not_found',
        recording_id,
        debug: { recording_id, fetchError: fetchError?.message || null, fetchCode: fetchError?.code || null },
      }
    }

    if (!recording.transcript) {
      return { status: 'no_transcript', recording_id }
    }

    // ═══ 1. KILL SWITCH (etapp 1b) ═══
    //
    // Lisas agentmotor respekterar business_config.agents_globally_paused
    // (app/api/agent/trigger/route.ts) — den SUGGESTING analysmotorn gjorde
    // det aldrig. En hantverkare som pausat agenterna fick ändå AI-förslag
    // och nya godkännandekort på varje samtal. Fail-soft: saknas kolumnen
    // (gammal miljö) eller läsningen misslyckas av annan anledning, körs
    // analysen som förut — hellre ett förslag för mycket än ett tenant som
    // tystnar helt på grund av ett schemafel.
    try {
      const { data: businessConfig } = await supabase
        ...
        .maybeSingle()
      if (businessConfig?.agents_globally_paused === true) {
        return {
          status: 'globally_paused',
          recording_id,
          message: 'Agenterna är pausade för det här företaget — samtalet analyserades inte.',
        }
      }
    } catch (pauseErr) {
      console.warn('[analyze-call] kunde inte läsa agents_globally_paused, kör ändå:', pauseErr)
    }

    // ═══ 2. ATOMISKT ANSPRÅK (v155) ═══
    //
    // Ersätter den gamla SELECT-före-INSERT-spärren, som bara läste
    // ai_suggestion och därför aldrig skyddade mötesgrenen (skriver
    // pending_approvals). claimed=false betyder att en annan körning redan
    // äger den här analysen — pågående ELLER redan klar, anspråket rensas
    // bara vid fel (best effort) eller explicit force.
    const { claimed, migrationMissing } = await claimCallAnalysis(supabase, recording_id, force)
    claimedHere = claimed

    if (!migrationMissing && !claimed) {
      return {
        status: 'already_claimed',
        recording_id,
        message: 'Analysen är redan gjord eller pågår redan för det här samtalet.',
      }
    }

    // Reservväg (migrationen inte körd än): den gamla dubbelkörningsspärren.
    // Analysen körs numera ALLTID automatiskt efter transkriberingen — men
    // UI:t har också en Analysera-knapp. Utan spärren hade knappen skapat en
    // andra uppsättning identiska kort (och betalat en Haiku-körning till).
    if (migrationMissing && !force) {
...
8. Svara ENDAST med JSON, ingen annan text före eller efter`
    ...
    try {
      ...
      console.warn('[analyze-call] kostnadsmätning misslyckades:', costErr)
    }
    ...
    if (kastade.length > 0) {
      console.warn(
        `[analyze-call] ${kastade.length} förslag utanför analysmotorns område kastades:`,
        kastade.map(k => k.type).join(', ')
      )
    }

    let resultat: AnalyseraSamtalResult
    ...
    if (arMote) {
      ...
      for (const k of kort) {
        ...
        if (kortFel) console.error('[analyze-call] möteskort kunde inte skapas:', kortFel.message)
        else skapadeKort++
      }
      ...
      for (const suggestion of suggestions) {
        ...
        if (suggestion.type === 'customer_fact') {
          ...
          if (faktaKortFel) console.error('[analyze-call] telefon-faktakort kunde inte skapas:', faktaKortFel.message)
          continue
        }
    ...
    // ═══ 3. ANALYSEN SLUTFÖRD ═══
    //
    // analyzed_at ≠ analysis_claimed_at — anspråket togs innan modellanropet,
    // det här sätts efter att korten faktiskt skapats. Ett anspråk utan
    // analyzed_at betyder "pågår eller kraschade", inte "klar".
    if (!migrationMissing) {
      await supabase
        .from('call_recording')
    ...
    // Släpp anspråket (best effort) — annars blockerar en transient krasch
    // (nätverksfel, modellfel) all framtida analys av det här samtalet.
    if (claimedHere) {
      try {
        await supabase
          .from('call_recording')
        ...
        console.error('[analyze-call] kunde inte släppa anspråket:', releaseErr)
      }
    }
    return {
      status: 'error',
