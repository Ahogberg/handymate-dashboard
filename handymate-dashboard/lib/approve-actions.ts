...
export async function executeApproveAction(
  ...
  switch (suggestion.suggestion_type) {
    ...
    case 'sms':
      return sendSMS(supabase, suggestion, actionData)
    case 'reminder':
      ...
      if (starkasteTraff) {
        ...
        const { data: newCustomer } = await supabase
          ...
          .insert({
            ...
            // v152 (kontaktproveniens): AI-förslag härlett ur ett
            // telefonsamtal (call_recording).
    ...
    if (!customerId && (actionData.customer_name || suggestion.call_recording?.phone_number)) {
      const { data: newCustomer } = await supabase
        ...
        .insert({
          ...
          address_line: actionData.address || null,
          // v152 (kontaktproveniens): se motivering i createBooking ovan.
          contact_source: 'phone_call',
          contact_source_at: new Date().toISOString(),
        })
        .select('customer_id')
...
async function createFollowUp(supabase: SupabaseClient, suggestion: any, actionData: any) {
  try {
    // Uppföljningen ska landa där någon faktiskt tittar. Tidigare skrevs den
    // till en kö utan läsare (bekräftad tyst läcka, se
    // tasks/rapport-human-followup-queue.md): godkännandet svarade
    // "schemalagd!" medan posten försvann. visibility 'team' är obligatoriskt
    // här — en rad utan skapare/tilldelad med 'private' syns inte för någon
    // (v42-regeln). Kontraktet låses av tests/followup-landar.spec.ts.
    const prioritet =
 ...
 *
 * Två saker ändrades utöver opt-out, sms_log och kostnadsmätningen:
 * avsändaren var hårdkodad till 'Handymate' i stället för hantverkarens eget
 * företagsnamn — kunden såg alltså fel avsändare — och auto-approve-vägen
 * (lib/auto-approve.ts) skickar hit UTAN att någon människa tittat, vilket är
 * precis det fall opt-out-spärren finns för.
 */
async function sendSMS(supabase: SupabaseClient, suggestion: any, actionData: any) {
  try {
    ...
    if (!r.success) {
      throw new Error(`SMS failed: ${r.error || 'okänt fel'}`)
    }
    return { success: true }
  } catch (error: any) {
    return { success: false, error: error.message }
  ...
  try {
    ...
    const { data: customer, error } = await supabase
      ...
      .insert({
        ...
        address_line: actionData.address || null,
        // v152 (kontaktproveniens): "create_customer"-förslaget kommer
        // liksom de andra AI-förslagen ovan från ett telefonsamtal.
        contact_source: 'phone_call',
        contact_source_at: new Date().toISOString(),
      })
      .select('customer_id')
...
async function rescheduleBooking(supabase: SupabaseClient, suggestion: any, actionData: any) {
  try {
    ...
    if (phoneNumber) {
      // Genom strypunkten (etapp 0 batch 2). Avsändaren var hårdkodad till
      // 'Handymate' — kunden såg alltså vår produkt som avsändare i stället
      // för sin hantverkare.
      ...
      if (!r.success) console.error('[approve-actions] ombokningsbekräftelse misslyckades:', r.error)
    }
