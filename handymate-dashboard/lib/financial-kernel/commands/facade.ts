...
import { executePaymentCommand, claimEffectIntents, finishEffectIntent } from './service'
...
export async function applyKernelPayment(opts:ApplyPaymentOptions):Promise<ApplyPaymentResult> {
  ...
  const out=await executePaymentCommand(db,{
  ...
  if(command.route==='legacy') return applyInvoicePaymentLegacy(opts)
