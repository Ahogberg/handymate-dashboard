...
export interface FinancialEventPayloads {
  invoice_issued: { invoice_id: string; invoice_number: string; customer_id: string; project_id?: string;
    currency: string; total_minor: string; vat_regime: 'standard' | 'reverse_charge_construction';
    accounting_method: 'accrual' | 'cash'; issued_date: string; due_date: string; tax_reduction?: 'rot' | 'rut' }
  invoice_credited: { invoice_id: string; credit_invoice_id: string; currency: string; amount_minor: string; issued_date: string }
  receivable_created: { receivable_id: string; invoice_id: string; component: 'customer' | 'tax_authority';
    owner: 'business' | 'factor'; currency: string; amount_minor: string; due_date: string }
  receivable_adjusted: { receivable_id: string; reason: 'credit' | 'write_off' | 'dunning_fee' | 'interest' | 'rounding' | 'ownership_transfer' | 'reclassification';
    delta_minor?: string; owner_after?: 'business' | 'factor'; from_component?: 'customer' | 'tax_authority'; to_component?: 'customer' | 'tax_authority' }
  ...
  payment_initiated: { payment_id: string; payment_intent_id?: string; provider: string; provider_ref?: string;
    direction: 'inbound' | 'outbound'; currency: string; amount_minor: string }
  payment_settled: { payment_id: string; currency: string; amount_minor: string; fee_minor?: string;
    settled_at: string; evidence: 'provider' | 'manual' | 'fortnox' | 'bank' }
  payment_allocated: { allocation_id: string; payment_id: string; receivable_id: string; currency: string; amount_minor: string }
