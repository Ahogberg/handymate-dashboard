import { buildQuoteSmsText } from './quote-sms'
...
export function buildQuoteDeliveryEnvelope(input: {
  ...
  const sms = recipients.smsTo ? { to: recipients.smsTo, text: buildQuoteSmsText({
    customerName: quote.customer.name, businessName: business.business_name,
    businessPhone: business.phone_number, assignedPhoneNumber: business.assigned_phone_number,
    total: quote.total, customerPays: quote.customer_pays, rotRutType: quote.rot_rut_type,
    validUntil: quote.valid_until, portalUrl,
  }) } : null
