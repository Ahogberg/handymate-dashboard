...
## 2026-09-18 — Gissningen var fel på varje punkt, och facit fanns en nedladdning bort
Bolagsverkets OpenAPI-fil visade att `parseOrganisationResponse` läste FEL nivå på varje fält (svaret är `{organisationer:[…]}`, adressen ligger under `postadressOrganisation.postadress`, näringsgrenen heter `naringsgrenOrganisation.sni`), att `identitetsbeteckning` är TIO siffror för ett org.nr (vi skickade `16`+tio), att en `identitetsbeteckning` kan bära FLERA organisationer, och att `organisationsnamnLista[0]` inte är firmanamnet. Uppslaget hade alltså aldrig kunnat lyckas — 404:an och 401:an dolde bara att nästa vägg stod strax bakom. Regeln: när en integration har en publicerad OpenAPI/Swagger ska den hämtas FÖRE första raden kod, och facit-testerna byggas på leverantörens egna `components.examples` i stället för påhittade svar. Ett API som inte går att nå härifrån är inget skäl att gissa — filen går att be användaren om.
